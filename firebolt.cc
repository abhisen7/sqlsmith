    ...
    if(curl){
      ...
      //  curl_easy_setopt(curl, CURLOPT_FTP_SKIP_PASV_IP, 1L);
       // curl_easy_setopt(curl, CURLOPT_TCP_KEEPALIVE, 1L);
        ...
        catch (dut::broken &e) {throw;}

        std::cerr << "Connected to Firebolt...\n\n\n***************************************************\n\n\n";
    }
}

CURLcode connect_fb::exec_query(const char* URI, const char* query, callback func, callback header){
        CURLcode ret;
...
schema_fb::schema_fb(std::string URI)
    ...
    /* Add functions listed under https://docs.firebolt.io/sql-reference/functions-reference/#sql-functions */
    ...
    FUNC1(sqrt,DOUBLE PRECISION,INTEGER); //to-do: Add NULL ret value for negative arg

    FUNC2(log,DOUBLE PRECISION,INTEGER, INTEGER); //to-do: Test with positive real numbers as base and exponent for arguments
    ...
    /* Aggregate Functions */
    // To-do: Expand AGG macro to functions with 2 arguments (e.g. MAX_BY, PERCENTILE_CONT) and array functions (ARRAY_AGG) 
    ...
    #define BIN1(n,r, a) do {						\
    routine proc("", "", sqltype::get(#r), #n);				\
    proc.argtypes.push_back(sqltype::get(#a));				\
    register_aggregate(proc);						\
    } while(0)

    #define BIN2(n,r, a,b) do {						\
    routine proc("", "", sqltype::get(#r), #n);				\
    proc.argtypes.push_back(sqltype::get(#a));              \
    proc.argtypes.push_back(sqltype::get(#b));					\
    register_aggregate(proc);						\
    ...
    generate_indexes();
    curl_easy_cleanup(curl); // tear down curl connection until DUT is initialized again
    curl=nullptr;

}

size_t dut_fb::dut_callback(char* ptr, size_t size, size_t resp_size, void* userdata){
...
size_t dut_fb::header_callback(char* buffer, size_t size, size_t nitems, void* userdata){
    if(c>0){
        (void) buffer; (void) size; (void) userdata;
        return nitems;
    }
    /*buffer is not null-terminated and has an additional new-line at the end of each header line, nitems includes the new-line*/
    buffer[nitems-1]='\0';
...
dut_fb::dut_fb(std::string URI)
    :connect_fb(URI.c_str(),0)
    {
